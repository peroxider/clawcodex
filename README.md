...
|---|---|---|
| `install.sh` / `install.ps1` | v0.5.0 | The installer scripts that ship with this README (released with the clawcodex tag they install) |
| `clawcodex` | v0.5.0 | The version the installers install |
| Git ref | `v0.5.0` | The git tag/branch the installers clone |
...
The fastest path for bash-capable environments. `install.sh` does everything end-to-end: OS detection,
...
**Development branch (latest, unpinned):**

```bash
# Replace {BRANCH} with the actual branch name (e.g. dev-decoupling-refactor-b24b8cb)
...
bash /tmp/clawcodex/install.sh --no-venv --no-setup --yes --log-file /tmp/install.log
# Install a specific tag / commit
bash /tmp/clawcodex/install.sh --ref v0.5.0
```

> 💡 **Windows users:** If you are on native PowerShell 5.1+ or pwsh, use the [PowerShell one-click install](#one-click-install-powershell) below instead. It works natively — no Git Bash or WSL required.
...
For native Windows environments (PowerShell 5.1+ or pwsh). `install.ps1` mirrors
every capability of `install.sh` — Git detection, uv provisioning, Python setup,
venv creation, lock-pinned dep install, global command registration — entirely
within PowerShell, with no Git Bash or WSL dependency.
...
```powershell
# Replace {BRANCH} with the actual branch name (e.g. 2026.6.25)
...
| OS | Status | Notes |
|---|---|---|
| Linux (Debian, Ubuntu, Fedora, RHEL, Arch, openSUSE, …) | ✅ Supported | Default test platform; one-click via `install.sh` |
| macOS 12+ (Monterey and newer) | ✅ Supported | Apple Silicon and Intel; one-click via `install.sh` |
| WSL2 (Ubuntu / Debian inside Windows) | ✅ Supported | One-click via `install.sh` inside WSL |
| Windows: native PowerShell 5.1+ / pwsh | ✅ Supported | One-click via `install.ps1` — no Git Bash or WSL needed |
| Git Bash on Windows | ✅ Supported | Alternative path; also uses `install.sh` |

> 💡 **Windows users:** You no longer need Git Bash or WSL. The PowerShell installer `install.ps1` handles everything natively — Git detection, uv provisioning, Python setup, venv creation, dependency install, and global command registration — all from a single `iwr | iex` one-liner. See [Quick Start](#one-click-install-powershell) below.
