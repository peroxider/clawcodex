...
Agent ``DashboardList``/``DashboardGet`` tools) speak the same shape when
reading progress data from goal, task, orchestrator, and SOP sources.

Design rules followed here:
  * typing.Protocol only — no ABC inheritance.
  * No implementation logic — only signatures + the immutable
    ``DashboardEntry`` dataclass.
  * No imports from Layer 1 (upstream). Consumers compose the protocol
...
class DashboardSource(Protocol):
    ...
    Sources MAY raise on transient errors; the DashboardStore
    converts raises into an empty snapshot for that source and logs
    the error so a single broken source does not blank the dashboard.
    """
