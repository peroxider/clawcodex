...
**Commit red lines (MUST follow):**
- **Never** add any of the following orchestrator-internal report files to a
  git commit: `changes_summary.md`, `implementation_notes.md`, `analysis.md`,
  `verification_report.md`. These are temporary reports generated by the
  orchestrator, not code changes.
- Before committing, explicitly `git add` only the source/test files you
  actually modified; **never use `git add .` / `git add -A`** (they sweep
  report files into the commit).
- If `git status` shows any of the report files above, ignore them — do not
  add, commit, or delete them.
