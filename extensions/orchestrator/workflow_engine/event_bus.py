...
class EventBus:
    ...
    def emit_stage_complete(self, stage_id: int, stage_name: str, cost: float = 0.0, duration: float = 0.0, **kwargs: Any) -> None:
        self.emit("stage_complete", stage_id=stage_id, stage_name=stage_name, cost=cost, duration=duration, **kwargs)
    ...
    def emit_decision(self, stage_id: int, outcome: str, next_stage: int | None = None, **kwargs: Any) -> None:
        self.emit("decision_evaluated", stage_id=stage_id, outcome=outcome, next_stage=next_stage, **kwargs)
