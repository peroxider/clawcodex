...
class AgentMarkdownWriter:
    ...
    def write_agent(
        ...
        use_lazy = len(raw_tools) > MAX_INLINE_TOOL_DISPLAY

        if is_domain_agent:
            render_tools = sorted(POS_SOP_DOMAIN_AGENT_TOOLS)
            skill_name = agent_type_to_skill_name(name)
            skills_list = agent_def.get("skills") or [skill_name]
        else:
            render_tools = sorted(POS_PROXY_BASE_TOOLS) if use_lazy else raw_tools
    ...
    def write_overview_agent(
        ...
        content = tmpl.render(
            ...
            proxy_tools=sorted(POS_PROXY_BASE_TOOLS),
            component_agents=component_agents,
            workflow_stages=workflow_stages,
        )
