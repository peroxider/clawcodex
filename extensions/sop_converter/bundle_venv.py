...
def bundle_venv_dir(bundle_dir: str | Path) -> Path:
    ...
    (``~/.cache/clawcodex/bundle-venvs/<hash>``) instead of ``/mnt/d/`` to
    avoid the extreme I/O penalty of the 9P bridge to Windows NTFS.
    Writing thousands of package files on ``/mnt/`` can be 10-100x slower
    than on the native filesystem.
    """

    bundle_path = normalize_runtime_path(bundle_dir)
    if is_wsl_runtime():
        key = hashlib.sha256(str(bundle_path).encode("utf-8")).hexdigest()[:16]
        return Path.home() / ".cache" / "clawcodex" / "bundle-venvs" / key
...
def _reset_venv_dir(venv_dir: Path, *, reason: str) -> None:
    ...
    # 2. ~/.cache/clawcodex/bundle-venvs/<hash> (WSL native ext4 layout)
