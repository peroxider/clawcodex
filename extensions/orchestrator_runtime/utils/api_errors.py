...
byte-identical (except this banner) to ``clawcodex_ext/services/api/
errors.py`` at Phase 2 publish time.

Phase 3 may shed unrelated helpers (e.g. ``is_media_size_error``,
``is_image_unsupported_error``) once AgentRunner only needs
``RateLimitError`` + ``is_rate_limit_error`` for retry logic.
"""
