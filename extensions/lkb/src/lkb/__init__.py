"""Logical Kanban agent-loop foundation (lkb standalone package)."""

from __future__ import annotations

from lkb.adapters import (
    maybe_commit_task_update,
    maybe_commit_todo_write,
    prepare_task_change,
    prepare_todo_write,
)
from lkb.audit import (
    AuditEvent,
    AuditEventType,
    AuditLog,
    EventDecision,
    InMemoryAuditLog,
    SessionFileAuditLog,
    append_event_once,
    append_proof_enrichment_once,
    default_session_log_path,
    event_for_acceptance_template_referenced,
    event_for_acceptance_template_registered,
    event_for_assumption_invalidated,
    event_for_commit,
    event_for_decomposition_proposed,
    event_for_external_config_imported,
    event_for_fact_dropped,
    event_for_fact_extracted,
    event_for_human_override,
    event_for_legacy_todo_ambiguity,
    event_for_llm_fallback_used,
    event_for_method_referenced,
    event_for_proof_enrichment,
    event_for_proposal,
    event_for_revalidation_requested,
    event_for_validation_run,
    get_audit_log,
)
from lkb.acceptance_template import (
    AcceptanceTemplate as TopLevelAcceptanceTemplate,
    AcceptanceTemplateRegistry,
    AcceptanceTemplateStatus,
    default_project_acceptance_templates_dir,
    default_user_acceptance_templates_dir,
    ensure_default_acceptance_template_dirs,
    get_acceptance_template,
    get_all_acceptance_templates,
    initialize_acceptance_template_registry,
    list_acceptance_templates,
    load_acceptance_template_data,
    load_acceptance_template_library,
    load_acceptance_template_library_layered,
    register_acceptance_template,
    reset_acceptance_template_registry,
    save_acceptance_template_library,
)
from lkb.acceptance_template_prompt import (
    AcceptanceTemplateSummaryResult,
    score_acceptance_template,
    select_templates_by_goal,
    summarize_acceptance_templates,
)
from lkb.decomposer import (
    DecompositionPlan,
    ProposedTask,
    TaskDecomposer,
    TaskDecompositionError,
)
from lkb.causal import (
    CAP_VERBS,
    Baseline,
    CausalEdge,
    CausalEffect,
    CausalEngine,
    CausalGraph,
    CausalMechanism,
    CausalScope,
    CausalTag,
    MODERATE_THRESHOLD,
    SIGNIFICANT_THRESHOLD,
    build_causal_graph,
    is_strict_causal_enabled,
    to_wire_json,
)
from lkb.commit_gate_fuzzy import (
    FUZZY_THRESHOLD_MINOR,
    aggregate_world_results,
    commit_gate_fuzzy_check,
)
from lkb.explain import (
    build_repair_suggestions,
    explain_issue,
    explain_validation_run,
    next_actions_for_task,
    proof_trace_summary,
)
from lkb.fuzzy_types import (
    AggregationAction,
    AggregationDecision,
    AggregationStrategy,
    Ambiguity,
    AmbiguityKind,
    AmbiguityReport,
    Assumption,
    AssumptionSource,
    Clarification,
    ClarificationAction,
    CommitDecision,
    DetectionMethod,
    Interpretation,
    MultiWorldResult,
    Severity,
    ValidationResultForWorld,
    World,
    WorldValidationResult,
)
from lkb.glossary import BUILT_IN_GLOSSARY, Glossary, GlossaryEntry
from lkb.flags import (
    CAUSAL_FEATURE_NAME,
    FEATURE_NAME,
    LLM_FACTS_FEATURE_NAME,
    is_causal_verification_enabled,
    is_llm_facts_enabled,
    is_logical_kanban_enabled,
)
from lkb.ambiguity_detector import AmbiguityDetector
from lkb.fuzzy_patterns import (
    BUILT_IN_PATTERN_LIBRARY,
    DomainConstraint,
    FuzzyPattern,
    FuzzyPatternLibrary,
)
from lkb.ir import (
    SCHEMA_VERSION,
    AssertionKind,
    AssertionRole,
    CanonicalAssertion,
    IROperation,
    IRNode,
    IRPredicate,
    IRVariable,
    Quantifier,
    and_,
    implies,
    make_canonical,
    not_,
    or_,
    pred,
)
from lkb.ir_hash import assertion_hash, canonical_hash, canonical_json
from lkb.ir_renderer import render_assertion, render_node, render_proof_trace
from lkb.llm_fact_extractor import LlmFactExtractor, extract_facts
from lkb.method_library import (
    AcceptanceTemplate,
    EngineeringMethod,
    METHOD_LIBRARY,
    SubtaskRole,
    SubtaskTemplate,
    default_lkb_cache_dir,
    default_user_methods_dir,
    ensure_default_dirs,
    get_all_methods,
    get_method,
    initialize_method_registry,
    list_methods,
    load_method_library,
    load_method_library_layered,
    register_method,
    reset_method_registry,
    save_method_library,
)
from lkb.method_prompt import (
    MethodSummaryResult,
    estimate_tokens,
    score_method,
    select_methods_by_pattern,
    summarize_methods,
)
from lkb.operation_schema import (
    OperationSchema,
    get_all_operation_schemas,
    get_operation_schema,
    load_operation_schema_data,
    register_operation_schema,
    register_operation_schemas,
    reset_operation_registry,
)
from lkb.ontology_graph import (
    OntologyGraph,
    get_registered_ontology,
    load_ontology_turtle,
    merge_ontology_graphs,
    register_ontology_graph,
    reset_ontology_registry,
)
from lkb.external_config import ConfigConflictError, ExternalConfigImporter, ImportResult
from lkb.external_config_lint import (
    LintIssue,
    LintReport,
    lint_all,
    lint_acceptance_templates,
    lint_cross_references,
    lint_method_library,
    lint_ontology,
    lint_operation_schema,
)
from lkb.metrics import (
    LLM_FACTS_DROPPED,
    LLM_FACTS_EXTRACTED,
    LLM_FALLBACK_USED,
    record_adapter_result,
    record_blocked_tasks,
    record_commit,
    record_denial,
    record_llm_facts_dropped,
    record_llm_facts_extracted,
    record_llm_fallback_used,
    record_stale_assumptions,
    record_snapshot_cache_hit,
    record_snapshot_cache_miss,
    record_timeout,
    record_validation_run,
    register_sink,
    reset_sinks,
)
from lkb.orchestrator import (
    acceptance_proof_required,
    latest_denial_for_task,
    read_audit_events_for_run,
    task_list_summary,
    task_ready_state,
    validate_task_transition,
)
from lkb.multiworld_validator import MultiWorldValidator
from lkb.predicate_extractor import (
    PredicateExtraction,
    extract_predicates,
    validate_assertion,
)
from lkb.rule_engine import (
    Layer1RuleEngine,
    RuleEngineResult,
    evaluate_rules,
    validate_acceptance_template_references,
    validate_external_config_references,
    validate_method_compliance,
)

# F-152: defer the scheduling_solver import until first access.
_SCHEDULING_NAMES = (
    "Resource",
    "Schedule",
    "SchedulingError",
    "SchedulingSolver",
    "SchedulingTask",
    "SchedulingUnavailable",
    "validate_schedule",
)


def __getattr__(name: str):  # type: ignore[no-untyped-def]
    if name in _SCHEDULING_NAMES:
        from lkb import scheduling_solver as _sched  # noqa: PLC0415

        value = getattr(_sched, name)
        globals()[name] = value
        return value
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


from lkb.runtime import LogicalKanbanRuntime, get_logical_kanban  # noqa: E402
from lkb.service import LogicalKanbanService  # noqa: E402
from lkb.solver_adapter import (  # noqa: E402
    AtpTptpSolverAdapter,
    ClingoSolverAdapter,
    DatalogSolverAdapter,
    Layer1SolverAdapter,
    LlmKnowledgeAdapter,
    SolverAdapter,
    SolverRequest,
    SolverResponse,
    Z3SolverAdapter,
    all_adapters,
    default_adapters,
    encode_solver_facts,
    encode_solver_literal,
    extended_adapters,
)
from lkb.atp import (  # noqa: E402
    Mace4SolverAdapter,
    Prover9SolverAdapter,
    VampireSolverAdapter,
    parse_mace4_interpretation,
    parse_szs_status,
)
from lkb.solver_limits import SolverLimitError, SolverResourceLimits, run_external_solver  # noqa: E402
from lkb.solver_pipeline import SolverPipeline  # noqa: E402
from lkb.truth_maintenance import (  # noqa: E402
    AssumptionRecord,
    AssertionRecord,
    TruthMaintenanceSystem,
)
from lkb.types import (  # noqa: E402
    CommitResult,
    DerivedStatus,
    FactsSnapshot,
    LkbStatus,
    Proposal,
    ProposedChange,
    RepairAction,
    RepairSuggestion,
    ValidationIssue,
    ValidationRun,
)
from lkb.world_generator import WorldGenerator  # noqa: E402

__all__ = [
    "AcceptanceTemplate",
    "AggregationAction",
    "AggregationDecision",
    "AggregationStrategy",
    "Ambiguity",
    "AmbiguityDetector",
    "AmbiguityKind",
    "AmbiguityReport",
    "AssertionKind",
    "AssertionRole",
    "Assumption",
    "AssumptionRecord",
    "AssumptionSource",
    "AtpTptpSolverAdapter",
    "AuditEvent",
    "AuditEventType",
    "AuditLog",
    "BUILT_IN_GLOSSARY",
    "BUILT_IN_PATTERN_LIBRARY",
    "EngineeringMethod",
    "CAP_VERBS",
    "CanonicalAssertion",
    "CausalEdge",
    "CausalEffect",
    "CausalEngine",
    "CausalGraph",
    "CausalMechanism",
    "CausalScope",
    "CausalTag",
    "Clarification",
    "ClarificationAction",
    "ClingoSolverAdapter",
    "CommitDecision",
    "CommitResult",
    "DecompositionPlan",
    "DatalogSolverAdapter",
    "DetectionMethod",
    "DerivedStatus",
    "DomainConstraint",
    "EventDecision",
    "FactsSnapshot",
    "FUZZY_THRESHOLD_MINOR",
    "FuzzyPattern",
    "FuzzyPatternLibrary",
    "Glossary",
    "GlossaryEntry",
    "IROperation",
    "IRNode",
    "IRPredicate",
    "IRVariable",
    "Interpretation",
    "InMemoryAuditLog",
    "Layer1RuleEngine",
    "Layer1SolverAdapter",
    "LLM_FACTS_DROPPED",
    "LLM_FACTS_EXTRACTED",
    "LLM_FALLBACK_USED",
    "LLM_FACTS_FEATURE_NAME",
    "LkbStatus",
    "LlmFactExtractor",
    "LlmKnowledgeAdapter",
    "LogicalKanbanRuntime",
    "LogicalKanbanService",
    "METHOD_LIBRARY",
    "SubtaskRole",
    "SubtaskTemplate",
    "Mace4SolverAdapter",
    "MultiWorldResult",
    "MultiWorldValidator",
    "MODERATE_THRESHOLD",
    "PredicateExtraction",
    "Prover9SolverAdapter",
    "Proposal",
    "ProposedChange",
    "ProposedTask",
    "Quantifier",
    "RepairAction",
    "RepairSuggestion",
    "RuleEngineResult",
    "Resource",
    "SCHEMA_VERSION",
    "Schedule",
    "SchedulingError",
    "SchedulingSolver",
    "SchedulingTask",
    "SchedulingUnavailable",
    "SIGNIFICANT_THRESHOLD",
    "SessionFileAuditLog",
    "Severity",
    "SolverAdapter",
    "SolverLimitError",
    "SolverPipeline",
    "SolverRequest",
    "SolverResourceLimits",
    "SolverResponse",
    "TaskDecomposer",
    "TaskDecompositionError",
    "TruthMaintenanceSystem",
    "ValidationIssue",
    "ValidationResultForWorld",
    "ValidationRun",
    "VampireSolverAdapter",
    "World",
    "WorldGenerator",
    "WorldValidationResult",
    "Z3SolverAdapter",
    "acceptance_proof_required",
    "append_event_once",
    "append_proof_enrichment_once",
    "aggregate_world_results",
    "all_adapters",
    "and_",
    "assertion_hash",
    "build_causal_graph",
    "CAUSAL_FEATURE_NAME",
    "FEATURE_NAME",
    "LLM_FACTS_FEATURE_NAME",
    "is_causal_verification_enabled",
    "is_llm_facts_enabled",
    "is_logical_kanban_enabled",
    "build_repair_suggestions",
    "canonical_hash",
    "canonical_json",
    "commit_gate_fuzzy_check",
    "default_adapters",
    "default_lkb_cache_dir",
    "default_session_log_path",
    "default_user_methods_dir",
    "encode_solver_facts",
    "encode_solver_literal",
    "evaluate_rules",
    "extended_adapters",
    "estimate_tokens",
    "event_for_assumption_invalidated",
    "event_for_commit",
    "event_for_decomposition_proposed",
    "event_for_fact_dropped",
    "event_for_fact_extracted",
    "event_for_human_override",
    "event_for_legacy_todo_ambiguity",
    "event_for_llm_fallback_used",
    "event_for_method_referenced",
    "event_for_proof_enrichment",
    "event_for_proposal",
    "event_for_revalidation_requested",
    "event_for_validation_run",
    "explain_issue",
    "explain_validation_run",
    "extract_facts",
    "extract_predicates",
    "get_audit_log",
    "get_logical_kanban",
    "get_all_methods",
    "get_method",
    "ensure_default_dirs",
    "initialize_method_registry",
    "implies",
    "is_strict_causal_enabled",
    "latest_denial_for_task",
    "list_methods",
    "load_method_library",
    "load_method_library_layered",
    "make_canonical",
    "maybe_commit_task_update",
    "maybe_commit_todo_write",
    "MethodSummaryResult",
    "next_actions_for_task",
    "not_",
    "register_method",
    "reset_method_registry",
    "or_",
    "parse_mace4_interpretation",
    "parse_szs_status",
    "pred",
    "prepare_task_change",
    "prepare_todo_write",
    "proof_trace_summary",
    "read_audit_events_for_run",
    "record_adapter_result",
    "record_blocked_tasks",
    "record_commit",
    "record_denial",
    "record_llm_facts_dropped",
    "record_llm_facts_extracted",
    "record_llm_fallback_used",
    "record_stale_assumptions",
    "record_snapshot_cache_hit",
    "record_snapshot_cache_miss",
    "record_timeout",
    "record_validation_run",
    "register_sink",
    "render_assertion",
    "save_method_library",
    "render_node",
    "render_proof_trace",
    "reset_sinks",
    "score_method",
    "select_methods_by_pattern",
    "run_external_solver",
    "summarize_methods",
    "task_list_summary",
    "task_ready_state",
    "to_wire_json",
    "validate_assertion",
    "validate_task_transition",
    "validate_method_compliance",
    "validate_schedule",
]

__all__.extend(
    [
        "AcceptanceTemplateRegistry",
        "AcceptanceTemplateStatus",
        "AcceptanceTemplateSummaryResult",
        "ConfigConflictError",
        "ExternalConfigImporter",
        "ImportResult",
        "LintIssue",
        "LintReport",
        "OperationSchema",
        "OntologyGraph",
        "TopLevelAcceptanceTemplate",
        "default_project_acceptance_templates_dir",
        "default_user_acceptance_templates_dir",
        "ensure_default_acceptance_template_dirs",
        "event_for_acceptance_template_referenced",
        "event_for_acceptance_template_registered",
        "event_for_external_config_imported",
        "get_acceptance_template",
        "get_all_acceptance_templates",
        "get_all_operation_schemas",
        "get_operation_schema",
        "get_registered_ontology",
        "initialize_acceptance_template_registry",
        "list_acceptance_templates",
        "lint_acceptance_templates",
        "lint_all",
        "lint_cross_references",
        "lint_method_library",
        "lint_ontology",
        "lint_operation_schema",
        "load_acceptance_template_data",
        "load_acceptance_template_library",
        "load_acceptance_template_library_layered",
        "load_operation_schema_data",
        "load_ontology_turtle",
        "merge_ontology_graphs",
        "register_acceptance_template",
        "register_operation_schema",
        "register_operation_schemas",
        "register_ontology_graph",
        "reset_acceptance_template_registry",
        "reset_operation_registry",
        "reset_ontology_registry",
        "save_acceptance_template_library",
        "score_acceptance_template",
        "select_templates_by_goal",
        "summarize_acceptance_templates",
        "validate_acceptance_template_references",
        "validate_external_config_references",
    ]
)