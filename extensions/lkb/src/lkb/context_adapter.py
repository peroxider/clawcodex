...
def _get_tms(context: Any):
    return getattr(_get_runtime(context), "tms", None)

def _snapshot_cache_key(context: Any) -> str:
...
def build_facts_snapshot(context: Any) -> FactsSnapshot:
...
def task_list_view(context: Any, *, include_lkb: bool = False) -> list[dict[str, Any]]:
...
def _latest_denial(context: Any, task_id: str) -> dict[str, Any] | None:
