...
``lkb/solver_atp.py``. Because the prover has
...
from lkb.context_adapter import build_facts_snapshot  # noqa: E402
from lkb.solver_adapter import (  # noqa: E402
    encode_solver_literal,
)
from lkb.solver_atp import (  # noqa: E402
    Clause,
    Literal,
    emit_tptp_program,
    pred,
...
from lkb.types import FactsSnapshot  # noqa: E402
...
class TestProverEndToEnd:
    ...
    def test_cycle_fails(self) -> None:
        v, _ = prove_lkb_request(
            constants=("A", "B"),
            blocked_ids=frozenset({"A", "B"}),
            cycle_ids=frozenset({"A", "B"}),
            has_proof_ids=frozenset(),
            completed_ids=frozenset(),
            strict_acceptance=False,
            proposal_target="A",
...
class TestDecisionMatrix:
    ...
    def test_blocked_task_fails_with_r002(self) -> None:
        ...
        snap = _snapshot(ctx)
        response = AtpTptpSolverAdapter().solve(_req(snap, target="B", status="in_progress"))
    ...
    def test_cycle_task_prefers_r006(self) -> None:
        ...
        response = AtpTptpSolverAdapter().solve(_req(snap, target="A", status="in_progress"))
        assert response.result == "fail"
    ...
    def test_completed_to_pending_reopens(self) -> None:
        ...
        response = AtpTptpSolverAdapter().solve(_req(_snapshot(ctx), target="A", status="pending"))
        assert response.result == "pass"
...
class TestQueryGuards:
    def test_unknown_target_id_denied(self) -> None:
        ctx = _ctx_with({"A": _task("A")})
        response = AtpTptpSolverAdapter().solve(_req(_snapshot(ctx), target="Z", status="pending"))
        assert response.result == "fail"
...
class TestServiceWithAtpPipeline:
    def test_service_can_swap_in_atp_pipeline(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ctx = _ctx_with({"A": _task("A"), "B": _task("B", blocked_by=["A"])})
        service = LogicalKanbanService()
        service.pipeline = SolverPipeline([Layer1SolverAdapter(), AtpTptpSolverAdapter()])
        snapshot = _snapshot(ctx)
        from lkb.runtime import get_logical_kanban
...
class TestTptpAudit:
    def test_last_tptp_program_is_well_formed(self) -> None:
        adapter = AtpTptpSolverAdapter()
        ctx = _ctx_with({"A": _task("A")})
        adapter.solve(_req(_snapshot(ctx), target="A", status="in_progress"))
        ...
        assert any("conjecture" in line and "do_proposal" in line for line in program.splitlines())
