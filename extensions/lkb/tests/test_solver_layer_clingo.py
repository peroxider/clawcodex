...
from lkb import (  # noqa: E402 - after importorskip
    ClingoSolverAdapter,
    Layer1SolverAdapter,
...
class TestDecisionMatrix:
    ...
    def test_blocked_task_fails_with_r002(self) -> None:
        ...
        snap = _snapshot(ctx)
        response = ClingoSolverAdapter().solve(_req(snap, target="B", status="in_progress"))
    ...
    def test_cycle_task_prefers_r006(self) -> None:
        ...
        response = ClingoSolverAdapter().solve(_req(snap, target="A", status="in_progress"))
        assert response.result == "fail"
    ...
    def test_completed_to_pending_reopens(self) -> None:
        ...
        response = ClingoSolverAdapter().solve(_req(_snapshot(ctx), target="A", status="pending"))
        assert response.result == "pass"
...
class TestQueryGuards:
    def test_unknown_target_id_denied(self) -> None:
        ctx = _ctx_with({"A": _task("A")})
        response = ClingoSolverAdapter().solve(_req(_snapshot(ctx), target="Z", status="pending"))
        assert response.result == "fail"
...
class TestServiceWithClingoPipeline:
    def test_service_can_swap_in_clingo_pipeline(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ctx = _ctx_with(
            {
                "A": _task("A"),
                "B": _task("B", blocked_by=["A"]),
        ...
        service.pipeline = SolverPipeline([Layer1SolverAdapter(), ClingoSolverAdapter()])
...
class TestLoggerSuppression:
    def test_atom_undefined_message_dropped(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        ClingoSolverAdapter().solve(_req(_snapshot(ctx), target="B", status="in_progress"))
    ...
    def test_real_grounding_error_still_surfaces(
        ...
        response = adapter.solve(_req(_snapshot(ctx), target="A", status="in_progress"))
