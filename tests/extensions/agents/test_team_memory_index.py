...
def test_recall_returns_scored_results(tmp_path: Path) -> None:
    ...
    q = TeamMemoryQuery(
        team_id="t1", query="deploy build", requester_agent_id="lead", top_k=3
    )
...
def test_source_filter(tmp_path: Path) -> None:
    ...
    store.append(
        TeamMemoryEntry(
            id="a", team_id="t1", content="manual note", summary="m",
            author_agent_id="lead", created_at=ts, source="manual", scope="team",
        )
    )
    store.append(
        TeamMemoryEntry(
            id="b", team_id="t1", content="manual note", summary="m",
            author_agent_id="lead", created_at=ts, source="system", scope="team",
        )
    )
    idx = TeamMemoryIndex(store)
    q = TeamMemoryQuery(
        team_id="t1", query="manual", requester_agent_id="lead",
        top_k=10, sources=("manual",),
    )
    results = idx.search(q)
...
def test_source_weight_ranks_manual_above_system(tmp_path: Path) -> None:
    ...
    manual = TeamMemoryEntry(
        id="m", team_id="t1", content="deploy checklist", summary="d",
        author_agent_id="lead", created_at=ts, source="manual", scope="team",
    )
    system = TeamMemoryEntry(
        id="s", team_id="t1", content="deploy checklist", summary="d",
        author_agent_id="lead", created_at=ts, source="system", scope="team",
    )
    store.append(manual)
    store.append(system)
    idx = TeamMemoryIndex(store)
    q = TeamMemoryQuery(
        team_id="t1", query="deploy", requester_agent_id="lead", top_k=2
    )
    results = idx.search(q)
...
def test_expired_entries_hidden_unless_include_expired(tmp_path: Path) -> None:
    ...
    entry = TeamMemoryEntry(
        id="x", team_id="t1", content="old", summary="o",
        author_agent_id="lead", created_at=past, expires_at=past, scope="team",
    )
    store.append(entry)
...
def test_empty_query_returns_no_results(tmp_path: Path) -> None:
    ...
    idx = TeamMemoryIndex(store)
    q = TeamMemoryQuery(
        team_id="t1", query="", requester_agent_id="lead", top_k=5
    )
