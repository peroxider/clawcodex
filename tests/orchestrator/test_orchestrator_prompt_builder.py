...
from extensions.orchestrator.workflow_store import WorkflowStore, get_workflow_store

# Minimal workflow content for TestRenderParts that contains the
# USER_MESSAGE_MARKER plus all expected system/user assertions.
_RENDER_PARTS_WORKFLOW = """\
---
agent:
  model: test-model
  provider: test
...
def setUp(self) -> None:
    ...
    def test_fallback_when_marker_missing(self) -> None:
        ...
        empty system string and the full prompt as user. This is the
...
class TestRulesReferenceInjection(unittest.TestCase):
    ...
    def test_prompt_injection_reference_line_present(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            wf_path = self._write_workflow(root, rules_enabled=True)
            store = get_workflow_store()
            store.load(wf_path)
    ...
    def test_prompt_injection_skipped_when_rules_disabled(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            wf_path = self._write_workflow(root, rules_enabled=False)
            store = get_workflow_store()
            store.load(wf_path)
            rendered = PromptBuilder.render(issue)
