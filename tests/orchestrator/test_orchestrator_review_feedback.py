...
class TestFilterPending(unittest.TestCase):
    ...
    def test_bot_login_excluded(self) -> None:
        result = self.service._filter_pending(
            self.record, [_feedback(author_login="clawcodex-bot")],
    ...
    def test_ignore_authors_excluded(self) -> None:
        ...
        result = service._filter_pending(
            self.record, [_feedback(author_login="ignored-user")]
        )
        self.assertEqual(result, [])
    ...
    def test_system_comment_dropped(self) -> None:
        ...
        result = self.service._filter_pending(
            self.record, [_feedback(body=body)]
        )
        self.assertEqual(result, [])
    ...
    def test_no_author_login_kept(self) -> None:
        ...
        result = self.service._filter_pending(
            self.record, [_feedback(author_login=None)],
            bot_login="clawcodex-bot",
        )
        self.assertEqual([f.id for f in result], ["fb-1"])
...
class TestCollectFollowups(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.registry = _build_registry(
            self.tmp.name,
            issues=[{"issue_id": "i1", "issue_identifier": "owner/repo#1",
                     "branch_name": "feat/1", "pr_number": 1}],
        )

    def test_disabled_returns_empty(self) -> None:
        tracker = _FakeTracker()
        service = ReviewFeedbackService(
            tracker=tracker, registry=self.registry,
    ...
    def test_max_feedback_items_per_run_truncates(self) -> None:
        ...
        feedback = [
            _feedback(id=f"fb-{i}", author_login="alice")
            for i in range(5)
        ]
    ...
    def test_cursor_set_to_updated_at(self) -> None:
        tracker = _FakeTracker(
            feedback_by_issue={
                "i1": [_feedback(id="fb-1", author_login="alice",
    ...
    def test_cursor_falls_back_to_id(self) -> None:
        tracker = _FakeTracker(
            feedback_by_issue={
                "i1": [_feedback(id="fb-99", author_login="alice",
