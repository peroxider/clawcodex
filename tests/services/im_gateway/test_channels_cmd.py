...
def test_channels_pair_verb_removed_from_normal_flow(capsys) -> None:
    rc = ch.run_channels_command(["pair", "wechat"])
    assert rc == 2
    assert "unknown channels subcommand" in capsys.readouterr().err


def test_channels_disconnect_verb_removes_wechat_gateway_connection(
    capsys,
    monkeypatch,
) -> None:
    calls: list[str] = []
    ...
    def _fake_disconnect(name: str, *, state_dir: str | None = None) -> int:
        calls.append(name)
        print("conversation connection removed")
        return 0

    monkeypatch.setattr(ch, "_disconnect_gateway_connection", _fake_disconnect, raising=False)

    rc = ch.run_channels_command(["disconnect", "wechat"])
...
def test_channels_command_dispatch(tmp_path, capsys, monkeypatch) -> None:
    p = tmp_path / "channels.yaml"
    ch.add_channel(str(p), _slack("s1", enabled=True))
    # status verb resolves through the default path; monkeypatch format_status
    # so dispatch is verified against the tmp config.
    monkeypatch.setattr(ch, "format_status", lambda _path, name=None: f"STATUS:{name or 'all'}")
    rc = ch.run_channels_command(["status", "s1"])
    assert rc == 0
    out = capsys.readouterr().out
    assert "STATUS:s1" in out
    # help
    assert ch.run_channels_command(["help"]) == 0
    # restart without name → error
    assert ch.run_channels_command(["restart"]) == 2


def test_channels_no_args_runs_guided_wizard(monkeypatch) -> None:
    called: list[str | None] = []

    def _fake_wizard(path: str | None = None) -> int:
        called.append(path)
        return 0

    monkeypatch.setattr(ch, "run_wizard", _fake_wizard)
    assert ch.run_channels_command([]) == 0
    assert called == [None]
