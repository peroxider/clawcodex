...
def test_config_response_language_override() -> None:
    assert (
        IntentForecastConfig.from_mapping({"response_language": "zh-CN"}).response_language
        == "Chinese"
    )
    assert (
        IntentForecastConfig.from_mapping({"response_language": "english"}).response_language
        == "English"
    )
    assert (
        IntentForecastConfig.from_mapping({"response_language": "klingon"}).response_language
        == "auto"
    )
...
def test_config_intent_strategy_is_one_of_supported_values() -> None:
    assert (
        IntentForecastConfig.from_mapping({"intent_strategy": "workspace"}).intent_strategy
        == "workspace"
    )
    assert (
        IntentForecastConfig.from_mapping({"intent_strategy": "history-first"}).intent_strategy
        == "history"
    )
    assert (
        IntentForecastConfig.from_mapping({"intent_strategy": "project"}).intent_strategy
        == "workspace"
    )
