...
class PortingWorkspaceTests(unittest.TestCase):
    ...
    def test_remote_mode_clis_run(self) -> None:
        remote_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "remote-mode", "workspace"],
            check=True,
            capture_output=True,
            text=True,
        )
        ssh_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "ssh-mode", "workspace"],
            check=True,
            capture_output=True,
            text=True,
        )
        teleport_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "teleport-mode", "workspace"],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_command_graph_and_tool_pool_cli_run(self) -> None:
        command_graph = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "command-graph"],
            check=True,
            capture_output=True,
            text=True,
        )
        tool_pool = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "tool-pool"],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_architecture_stats_cli_runs(self) -> None:
        ...
        for expected in ("Generator loop", "Tools", "Memory", "Hooks", "Rendering engine", "MCP"):
            self.assertIn(expected, result.stdout)

    def test_bootstrap_graph_and_direct_modes_run(self) -> None:
        graph_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "bootstrap-graph"],
            check=True,
            capture_output=True,
            text=True,
        )
        direct_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "direct-connect-mode", "workspace"],
            check=True,
            capture_output=True,
            text=True,
        )
        deep_link_result = subprocess.run(
            [sys.executable, "-m", "scripts.audit.main", "deep-link-mode", "workspace"],
            check=True,
            capture_output=True,
            text=True,
        )
