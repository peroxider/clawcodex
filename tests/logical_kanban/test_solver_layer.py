...
from clawcodex_ext.logical_kanban.solver_pipeline import _merge_responses
from clawcodex_ext.logical_kanban.types import FactsSnapshot, ValidationIssue
...
class TestServiceSolverIntegration:
    def test_service_uses_pipeline_for_transition(self, service: LogicalKanbanService, empty_context: ToolContext) -> None:
        _add_task(empty_context, 'A', status='pending')
    ...
    def test_service_denies_blocked_transition_with_pipeline(self, service: LogicalKanbanService, empty_context: ToolContext) -> None:
        _add_task(empty_context, 'A', status='pending')
        _add_task(empty_context, 'B', status='pending', blocked_by=['A'])
        change = ProposedChange(
            kind='transition_status',
    ...
    def test_service_denies_strict_acceptance_via_pipeline(self, service: LogicalKanbanService, empty_context: ToolContext) -> None:
    ...
    def test_service_unknown_solver_result_denies_commit(self, service: LogicalKanbanService, empty_context: ToolContext) -> None:
        _add_task(empty_context, 'A', status='pending')
