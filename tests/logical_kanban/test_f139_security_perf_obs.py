...
class TestObservabilityMetrics:
    ...
    def test_adapter_result_metric_emitted_on_timeout(self, empty_context, collector) -> None:
        _add_task(empty_context, "A", status="pending")
...
class TestCorrectness:
    def test_lkb_failure_does_not_corrupt_context_tasks(self, service, empty_context) -> None:
        _add_task(empty_context, "A", status="pending")
        _add_task(empty_context, "B", status="pending", blocked_by=["A"])
...
class TestPerformance:
    ...
    def test_task_update_overhead_under_50ms(self, empty_context) -> None:
        ...
        assert elapsed_ms < 50 * _ci_multiplier(), f"Task update overhead took {elapsed_ms:.1f}ms"
...
class TestHumanOverrideAudit:
    def test_override_clarification_emits_audit_event(self, service, empty_context) -> None:
