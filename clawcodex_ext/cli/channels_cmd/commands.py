"""``clawcodex-dev channels`` subcommand — guided IM channel configuration.

Three user-facing entries:
  clawcodex-dev channels                 Guided wizard (interactive menu)
  clawcodex-dev channels status [name]   Show channel health/status
  clawcodex-dev channels disconnect wechat
                                          Remove the REPL/orchestrator connection
  clawcodex-dev channels restart <name>  Rebuild a channel adapter

The wizard reads/writes ``~/.clawcodex/im-gateway/channels.yaml``
...
_USAGE = (
    "usage: clawcodex-dev channels [status [name] | restart <name> | help]\n\n"
    "  (no args)        Guided configuration wizard.\n"
...
def restart_channel(name: str, *, state_dir: str | None = None) -> int:
    ...
    print("Start the daemon with `clawcodex-dev gateway server start` for live reload.")
    return 0
    ...
    if data.get("bot_token"):
        ...
        if start_rc != 0:
            print(
                ...
                "  请手动执行 `clawcodex-dev gateway server start` 后重试 `channels restart "
                f"{name}`。",
                file=sys.stderr,
            )
            return start_rc
...
def _wizard_add(cfg, path, input_fn: InputFn) -> None:
    ...
    cfg.replace_channel(channel)
    save_config(cfg, path)
    print(f"已保存渠道 {name!r}。执行 `clawcodex-dev channels restart {name}` 生效。")
...
@register("channels")
def run_channels_command(args: list[str]) -> int:
    if not args:
    ...
    if verb == "status":
        ...
        print(format_status(None, name))
        return 0
    if verb in ("disconnect", "unbind"):
        if not rest:
            print("error: channels disconnect <wechat-name>", file=sys.stderr)
            return 2
        return _disconnect_gateway_connection(rest[0])
    if verb == "restart":
        if not rest:
            print("error: channels restart <name>", file=sys.stderr)
            return 2
        return restart_channel(rest[0])
    if verb == "login":
        if not rest:
            print("error: channels login <wechat-name>", file=sys.stderr)
            return 2
        return wechat_login(rest[0])
    if verb == "wizard":
        return run_wizard(None)
    print(f"error: unknown channels subcommand {verb!r}", file=sys.stderr)
    return 2
