...
def _add_scope_to_servers(
    ...
    if not servers:
        return {}
    return {
        name: ScopedMcpServerConfig(config=config, scope=scope) for name, config in servers.items()
    }
...
def _expand_env_vars(
    ...
    if isinstance(config, McpStdioServerConfig):
        ...
        expanded_headers = (
            {k: _expand(v) for k, v in config.headers.items()} if config.headers else None
        )
...
def get_mcp_configs_by_scope(
    ...
    if scope == "local":
        ...
        if result.config is None:
            non_missing = [
                e for e in result.errors if not e.message.startswith("MCP config file not found")
            ]
            return {}, non_missing
...
def filter_mcp_servers_by_policy(
    ...
    if _policy_flag("allow_managed_only_mcp", "allowManagedOnlyMcp"):
        kept = {
            name: cfg for name, cfg in configs.items() if cfg.scope in ("enterprise", "managed")
        }
