...
class TestReadNormalisedLoose:
    def test_crlf_only_difference_collapses_under_loose(self, tmp_path: Path) -> None:
        ...
        assert PatchGenerator.read_normalised(upstream) == PatchGenerator.read_normalised(
            downstream
        )
        # Loose mode also equal — superset of strict.
        assert PatchGenerator.read_normalised_loose(
            upstream
...
class TestFilesDifferNorm:
    ...
    def test_loose_true_treats_quote_only_as_equal(self, tmp_pair: tuple[Path, Path]) -> None:
        upstream, downstream = tmp_pair
        _write(upstream, b"name = 'alice'\n")
        _write(downstream, b'name = "alice"\n')

        assert PatchGenerator.files_differ_norm(upstream, downstream, loose=True) is False

    def test_loose_true_still_catches_real_change(self, tmp_pair: tuple[Path, Path]) -> None:
        upstream, downstream = tmp_pair
        _write(upstream, b"x = 'foo'\n")
        _write(downstream, b'x = "bar"\n')

        assert PatchGenerator.files_differ_norm(upstream, downstream, loose=True) is True
