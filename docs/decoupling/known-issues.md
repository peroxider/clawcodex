...
**问题**: P3-step4 创建的 `clawcodex_ext/bridge/repl_bridge_transport.py` 是 2 行 `from X import *` 薄 facade，源是 `src.bridge.repl_bridge_transport`（400+ 行真实现：`ReplBridgeTransport` Protocol、`V2TransportOptions`、v1/v2 transport 类与工厂）。

同样情况：`src/bridge/session_runner.py`（P3-step3 改造后已无 import，但仍 re-export `extensions/ports/bridge/session_runner.py`）。

**P3 处理**: 仅创建薄 facade 让 Layer 2 不直接 import Layer 0（满足 P3 规则：Layer 1 可 import Layer 0）。

**完整迁移范围**:
1. 把 `src.bridge.repl_bridge_transport` 400+ 行完整复制到 `clawcodex_ext/bridge/repl_bridge_transport.py`
2. 调整 import 链（如果 `src.bridge.repl_bridge_transport` 内部 import 其他 src 模块）
3. 把 `src.bridge.repl_bridge_transport` 改为薄 facade 指向 `clawcodex_ext.bridge.repl_bridge_transport`（反向 P3 模式）
4. 同理处理 `src.bridge.session_runner`（如果还没处理）

**预计工作量**: 1-2 天（含 import 链调整 + 回归测试）。优先级 P3（low — 当前 P3 范围已满足 100% 清洗）。
