...
- [附录 F：完整 tests 目录归属矩阵（权威清单）](#附录-f完整-tests-目录归属矩阵权威清单)

---

# Layer 1 — clawcodex_ext/（下游补丁层）

> Layer 1 合计 **7 个特性** / 50 个子目录 / 1,008 个 .py 文件 / **238,491 行实现** + 195,660 行测试代码（L1 归属部分） = **~434,151 行**。
>
> 注：`clawcodex_ext/community_radar/`（L1-⑧）整体归入 Layer 2（L2-⑨），**不在 Layer 1 规划范围**。
>
> 9 人平摊 → 形式上平均 **~26.5K 行实现/人**；impl+测试合计平均 **~48.2K / 人**。W7 单一小特性 L1-④ 仅有 ~13K impl，**当前实际最大-最小比 ≈ 3.5×**（W7 工作量待后续 Layer 2 规划时补齐，本文档暂不均摊）。

## 1.1 分配概览

| Worker | 主特性 | 负责子目录 | 实现行数 | 测试行数 | **合计** | 跨特性数 |
|---|---|---|---:|---:|---:|---:|
| **W1** | L1-① | `agent/`、`tool_system/`、`remote/` | **34,260** | 18,727 | **~52,987** | 1 |
| **W2** | L1-① | `command_system/`、`query/`、`bridge/`、`transports/`、`providers/`、`types/` | **34,017** | 24,474 | **~58,491** | 1 |
| **W3** | L1-① | `services/mcp/`、`voice/`、`channels/`、`im_gateway/` + `buddy/` | **25,294** | ~6,151 | **~31,445** | 1 |
| **W4** | L1-① | `services/{compact,templates,tool_execution,ultraplan,…}` （22 个子目录 + 8 个顶层文件） | **31,175** | 30,696 | **~61,871** | 1 |
| **W5** | L1-② | `cli/`、`cli_core/`、`entrypoints/`、`daemon/`、`native/`、`runtime/` | **14,540** | 18,178 | **~32,718** | 1 |
| **W6** | L1-③ | `tui/`、`repl/`、`frontend/`、`diagnostics/`、`debug/` | **33,526** | 18,376 | **~51,902** | 1 |
| **W7** | L1-④ | `permissions/`、`auth/`、`hooks/`、`bootstrap/` | **12,952** | ~7,369 | **~20,321** | 1 |
| **W8** | L1-⑤ | `skills/`、`context_system/`、`goal/`、`intent_forecast/`、`multimodel/`、`memdir/`、`away_summary/`、`dreaming/`、`coordinator/`、`session_intelligence/`、`logical_kanban/`、`memory/` | **33,887** | 37,054 | **~70,941** | 1 |
| **W9** | L1-⑥ + L1-⑦ | `utils/`、`cron_system/`、`tasks/`、`configuration/`、`feature_gate/`、`settings/`、`state/`、`orchestrator/`、`messaging/`、`assistant/`、`compact_service/`、`models/` | **18,840** | 34,635 | **~53,475** | 2 |
| **合计** | — | — | **238,491** | **195,660** | **~434,151** | — |

**平衡指标（暂不均衡）**：

- 平均工作量：~48.2K / 人（impl + 测试，**只统计 L1 归属**）
- L1 测试合计：195,660 行 = 280,941（tests/ 总） - 51,021（L2 延后） - 33,946（跨工人 / 待拆） - 314（LOCK 共享）
- 中位数工作量：~52K / 人
- 最大值：W8（~70,941）；最小值：W7（~20,321）
- 最大-最小比：70,941 / 20,321 ≈ **3.49×**（**超出** 2× 软阈值，**待 Layer 2 规划时为 W7 补充分配**）
...
**测试代码归属（18,727 行 / 8 个子目录 + 2 个顶层文件）— W1 独占，其他工人不修改**：
...
| `tests/clawcodex_ext/agent_tests/` | **(整目录)** — `clawcodex_ext/agent/` mirror 测试 | 523 | ✅ |
| `tests/clawcodex_ext/tool_system/` | **(整目录)** — `clawcodex_ext/tool_system/` mirror 测试 | 308 | ✅ |
| `tests/clawcodex_ext/agent/` | **(整目录)** — agent 子模块测试 | 247 | ✅ |
| `tests/test_agent_name.py` | **(整文件)** — Agent 命名 smoke | 453 | ✅ |
| `tests/test_session_chain.py` | **(整文件)** — F-103 parentUuid chain 测试 | 496 | ✅ |
...
**测试代码归属（24,474 行 / 7 个子目录 + 1 个顶层文件）— W2 独占，其他工人不修改**：
...
| `tests/plugin/` | **(整目录)** — pluggy 插件适配 / 加载器 / 依赖 / 生命周期 | 2,759 | ✅ |
| `tests/command_system/` | Slash 命令测试 | 2,170 | ✅ |
| `tests/clawcodex_ext/providers/` | **(整目录)** — `clawcodex_ext/providers/` mirror 测试（Kimi / stream_drain / F-99 registration） | 762 | ✅ |
| `tests/transports/` | CCR Transport 测试 | 491 | ✅ |
| `tests/clawcodex_ext/query/` | **(整目录)** — `clawcodex_ext/query/` mirror 测试（hook_registry / outbox_types / recovery_strategies） | 411 | ✅ |
| `tests/test_llm.py` | **(整文件)** — `clawcodex_ext/llm.py` 顶层模块 smoke | 186 | ✅ |
...
**测试代码归属（30,696 行 / 2 个整目录）— W4 独占，其他工人不修改**：

| 测试路径 | 职责 | 行数 | 独占 |
|---|---|---:|---|
| `tests/services/` | **(整目录)** — 全部 services 子系统聚合测试（swarm / oauth / advisor / compact / ultraplan / templates / tool_execution / analytics / pipe_ipc / lodestone / mcp / voice / channels / im_gateway 等） | 29,583 | ✅ |
| `tests/clawcodex_ext/services/` | **(整目录)** — `services/lodestone/` 等 mirror 镜像测试 | 1,113 | ✅ |
...
**测试代码归属（18,178 行 / 4 个子目录 + 1 个 mirror）— W5 独占，其他工人不修改**：

| 测试路径 | 职责 | 行数 | 独占 |
|---|---|---:|---|
| `tests/stability_gate/` | **(整目录)** — Stage 1-6 全部（smoke / perf / CLI / REPL 冷启动） | 9,040 | ✅ |
| `tests/cli/` | **(整目录)** — CLI dispatch / runners / subcommand | 7,180 | ✅ |
| `tests/trae/` | **(整目录)** — ACP CLI adapter / protocol / MCP bridge | 899 | ✅ |
| `tests/runtime/` | **(整目录)** — 运行时命令 | 596 | ✅ |
| `tests/clawcodex_ext/native/` | **(整目录)** — `clawcodex_ext/native/` mirror 测试（audio / image / modifiers / registry / url_handler） | 463 | ✅ |
...
**测试代码归属（37,054 行 / 12 个子目录 + 1 个 mirror）— W8 独占，其他工人不修改**：
...
| `tests/clawcodex_ext/logical_kanban/` | **(整目录)** — `clawcodex_ext/logical_kanban/` mirror 测试 | 245 | ✅ |
...
**测试代码归属（34,635 行 / 30 个子目录 + 1 个 mirror）— W9 独占，其他工人不修改**：

**注**：原 L1-⑥/L1-⑦ 部分归属测试目录（如 `tests/voice/`、`tests/messaging/`、`tests/abort/`）已改为整目录归属 W9，避免文件级拆分；`tests/diagnostics/` 归 W6（其 `clawcodex_ext/diagnostics/` impl owner），不在 W9。`tests/telemetry/` 与 `tests/clawcodex_ext/tasks/` 按 feature 亲和性归 W9（与 `state/`、`tasks/`、`messaging/` 同一基础设施层）。

| 特性 | 测试路径 | 职责 |
|---|---|---|
| L1-⑥ | `tests/cron/` | 整目录归 W9（分布式锁 Cron） |
| L1-⑥ | `tests/telemetry/` | 整目录归 W9（Telemetry 聚合 / analytics bridge / push errors） |
...
| L1-⑥ | `tests/clawcodex_ext/tasks/` | 整目录归 W9（`clawcodex_ext/tasks/` mirror — bg_session） |
...
| `tests/services/` | 29,583 | **W4**（独立 stub；含 5,063 行 services.mcp / 5,960 行 services.voice / 5,495 行 services.channels / 4,435 行 services.im_gateway 的间接覆盖） |
| `tests/cli/` | 7,180 | **W5** |
...
# 附录 F：完整 tests 目录归属矩阵（权威清单）

> **作用**：与 [附录 D](#附录-d完整-impl-文件归属矩阵逐子目录权威清单)（impl）配对，**`tests/` 全量盘点**——逐子目录、逐文件、按 worker 分组的权威归属清单。
>
> **基线**：`tests/` 共 **88 个子目录 + 5 个顶层文件 / 280,941 行 / ~660 个 .py**（2026-07-25 盘点）。本附录已 100% 覆盖校验（`UNASSIGNED=NONE, PHANTOM=NONE`）。
>
> **数据生成器**：`/tmp/gen_test_manifest.py`（同步数据，附录 B.3 是其精简版）。
>
> **注**：`tests/clawcodex_ext/` 和 `tests/extensions/` 是 `clawcodex_ext/`、`extensions/` 的 **子目录镜像**——已经按子目录拆分到对应 worker 或 L2（如 `tests/clawcodex_ext/agent/` 归 W1，`tests/extensions/recording/` 归 L2）。


### W1 — L1-① Agent & Tool（tests 合计 17,778 行 / 9 项）
...
### W2 — L1-① Command/Query/Bridge/Providers（tests 合计 24,288 行 / 8 项）
...
### W3 — L1-① Services Group A（tests 合计 5,870 行 / 3 项）
...
### W4 — L1-① Services Group B（tests 合计 30,696 行 / 2 项）
...
### W5 — L1-② CLI 与入口层（tests 合计 18,178 行 / 5 项）
...
### W6 — L1-③ TUI/REPL/前端（tests 合计 18,376 行 / 5 项）
...
### W7 — L1-④ 权限/鉴权/钩子（tests 合计 7,369 行 / 5 项）
...
### W8 — L1-⑤ 智能子系统（tests 合计 37,054 行 / 13 项）
...
### W9 — L1-⑥⑦ 调度/基础设施（tests 合计 34,635 行 / 31 项）
...
| 文件 | 归属 | 行数 |
|---|---|---:|
| `tests/test_session_chain.py` | **W1** | 496 |
| `tests/test_agent_name.py` | **W1** | 453 |
| `tests/test_mcp_ext.py` | **W3** | 281 |
| `tests/test_llm.py` | **W2** | 186 |
| `tests/conftest.py` | **LOCK** | 146 |

### L2 / 延后阶段（Layer 2 工人接手后再分配）

| 归属路径 | 行数 | 备注 |
|---|---:|---|
| `tests/orchestrator` | 34,202 | L2 |
| `tests/extensions/recording` | 3,948 | L2 |
| `tests/test_visualizer` | 3,161 | L2 |
| `tests/extensions/daemon` | 1,999 | L2 |
| `tests/api` | 1,583 | L2 |
| `tests/remote_api` | 1,450 | L2 |
| `tests/extensions/agent_dashboard` | 1,148 | L2 |
| `tests/server` | 1,142 | L2 |
| `tests/upstreamproxy` | 792 | L2 |
| `tests/extensions/agents` | 601 | L2 |
| `tests/sop_converter` | 402 | L2 |
| `tests/extensions/capabilities` | 177 | L2 |
| `tests/visualizer` | 170 | L2 |
| `tests/extensions/tool_system_ext` | 156 | L2 |
| `tests/upstream_sync` | 90 | L2 |

### 跨工人 / 待拆（**无清晰单一 owner**，Phase 1 不强制分配）

| 归属路径 | 行数 | 备注 |
|---|---:|---|
| `tests/misc` | 22,618 | 待拆 |
| `tests/parity` | 5,758 | 待拆 |
| `tests/integration` | 5,570 | 待拆 |

### LOCK / 共享（**团队负责人锁定**，所有 worker 不修改）
