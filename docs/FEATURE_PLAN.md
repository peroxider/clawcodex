...
> F-43 CLI 模型供应商与模型切换已在 clawcodex_ext/cli/model_cmd/ 完整落地（ModelRegistry / ModelStore / Resolver / RuntimeContext.swap_provider）。详细设计（动机、实现设计、1-5 设计要点、文件变更、验收）已归档至 [ARCHIVED_FEATURES.md §二十三.3 F-43 CLI 模型供应商与模型切换](./ARCHIVED_FEATURES.md#二十三3-f-43-cli-模型供应商与模型切换)。
    # ... 已有字段 ...
...
> 详细设计已归档。当前状态：✅ 基础完成（`clawcodex_ext/cli/dispatch.py` 的 `_resolve_startup_agent()` 完整实现 `--agent <name>` CLI 标志 + `.claude/agents/<name>.md` 自动发现 + 启动 banner；`pos convert` 输出格式兼容 `.claude/agents/<name>.md` 加载格式。REPL 内 `@agent-name` 引用自动解析。Headless/API/TUI 入口待补齐。）
> 
> 详细内容（背景与问题、目标、现状诊断、接入点设计、实现切片、验收标准、风险与约束、依赖与协同）请参见 [ARCHIVED_FEATURES.md §F-89 @agent-name 多入口统一支持](./ARCHIVED_FEATURES.md#f-89-agent-name-多入口统一支持)。
...
| F-97-J | 桥接 `SessionAnalyticsMetadata` / `collect_session_metadata` 到 telemetry `session_start` 字段 | ✅ 已完成（2026-06；commit `76d9688`，recorder 内部自动调 `collect_session_metadata`，6 埋点入口零改动，`extra` dict 走 redactor `_BLOCKED_EXTRA_KEYS` 阻断） |
| F-97-K | 支持从项目 TOML 配置文件加载 `[telemetry]` 段（FEATURE_PLAN §9.3 示例的对应实现） | ✅ 已完成（2026-06；commit `41e2b30`，`_toml.py` + `pyproject.toml [tool.clawcodex.telemetry]` walk-upward + `<cwd>/telemetry.toml`，JSON 仍胜出，`telemetry.toml.example` 进仓库根） |
| F-97-L | `SCHEMA_VERSION=1` → v2 升级路径（事件格式迁移、版本协商、降级读 v1） | ✅ 已完成（2026-06；commit `41e2b30`，`migration.py` 含 `migrate_v1_to_v2` + `normalize_event` + `_fingerprint_dict_to_hash`；`fingerprint` 字符串 → 结构化 dict；summary stamp v2；redaction 接受 v2 dict 形态；跨版本 fingerprint dedup 隐私断言） |
