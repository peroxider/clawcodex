...
**Phase 1 (本季度, 1-2 月)** — 立竿见影组
- DC-005 / DC-009 / DC-020 通过 CLAUDE.md + Hook + 输出风格约束立即落地
- DC-012 / DC-018 通过 prompt 模板落地
- 验证指标：用户报告"幻觉明显减少"

**Phase 2 (下季度, 2-4 月)** — 工具化组
- DC-006 / DC-003 / DC-008 / DC-010 借助 F-118 子 agent 编排落地
- DC-002 / DC-007 / DC-015 借力 F-119 Registry 落地
- 验证指标：关键决策类任务可观察对抗/多视角结果

**Phase 3 (半年后)** — 基础设施组
- DC-014 (CaC) → DC-016 (Marketplace) → DC-019 (Stress Test) 顺序构建
- DC-001 / DC-013 / DC-017 配合基础设施逐步落地
...
| 编号 | 名称 | 组别 | 核心杠杆 | 落地门槛 |
|:----:|------|------|:--------:|:--------:|
| DC-001 | 上下文模式热切换 | 生命周期 | 🟡 | 中 |
| DC-002 | 上下文继承链 | 生命周期 | 🟡 | 中 |
| DC-003 | JIT 上下文合成 | 生命周期 | 🔴🔴 | 低 |
| DC-004 | 记忆分层 (W/E/S) | 生命周期 | 🟢 | 低-高 |
| DC-005 | 置信度声明协议 | 抗幻觉 | 🔴🔴🔴 | 低 |
| DC-006 | 工具强制验证 | 抗幻觉 | 🔴🔴 | 低-中 |
| DC-007 | 自相矛盾检测 | 抗幻觉 | 🔴 | 中 |
| DC-008 | 对抗质疑器 | 抗幻觉 | 🔴🔴 | 中 |
| DC-009 | 否定式检索 | 抗幻觉 | 🔴🔴🔴 | 低 |
| DC-010 | 多视角扇出 | 推理扩展 | 🔴🔴 | 中-高 |
| DC-011 | 假设并行情景 | 推理扩展 | 🟡 | 中 |
| DC-012 | 反事实推理 | 推理扩展 | 🔴 | 低 |
| DC-013 | 类比迁移 | 推理扩展 | 🟡 | 中 |
| DC-014 | 上下文即代码 (CaC) | 元架构 | 🟡 | 高 |
| DC-015 | 上下文时序回放 | 元架构 | 🟡 | 中 |
| DC-016 | 上下文市场 | 元架构 | 🟢 | 高 |
| DC-017 | 认知模式混合 | 元架构 | 🟡 | 中 |
| DC-018 | 涌现式上下文发现 | 元架构 | 🔴 | 低 |
| DC-019 | 上下文压力测试 | 元架构 | 🟢 | 高 |
| DC-020 | 边界追踪 | 元架构 | 🔴🔴🔴 | 低 |