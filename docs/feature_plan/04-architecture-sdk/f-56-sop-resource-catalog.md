...
| `mark_failed` / `latest` / `find_by_source_tool` / `delete` / `list_keys` | ❌ YAGNI 已删；主路径仅 `upsert`/`get`/`find_*` | 否 |
...
主路径仅以上方法。`latest` / `find_by_source_tool` / `mark_failed` / `delete` / `list_keys` 曾实现但无生产调用，已按 YAGNI 删除；需要时再加。
高层入口：
...
| `mark_failed` / `latest` / `find_by_source_tool` / `delete` / `list_keys` | ❌ YAGNI 已删 |
| 专用 F-56 CLI | ❌ |
...
7. （可选）按需恢复 `mark_failed` / `latest` / 诊断 CLI 等查询 API（曾实现后因无生产调用按 YAGNI 删除）。
...
| fallback（F-55/F-56） | `get(type, id)` / `find_by_*`；句柄读序见 §4.5（`latest(type)` 未实现，需时再加） |
