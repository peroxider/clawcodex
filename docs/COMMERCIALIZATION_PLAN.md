...
clawcodex_ext/     Layer 1 — 下游补丁/扩展层（猴补丁、注册模式）
...
| Layer 1 | `clawcodex_ext/` | 下游补丁/扩展层 | 1,075 | 262,658 |
...
将 `src/` + `clawcodex_ext/` + `extensions/` 全部 squash 成一个独立商业仓库，但**不断开上游跟踪**。通过 `git remote add upstream <clawcodex>` + 定期 `git pull upstream main` 的方式持续获取上游更新，并将二开带来的冲突以补丁形式维护。

工作流：
```
1. 初始：git clone clawcodex → 合入全部二开代码 → 建立商业仓库
2. 定期：git fetch upstream → git merge upstream/main → 解决冲突 → 提交
3. 冲突处理：每次 merge 后，将 694 个被修改文件的冲突逐一解决，生成补丁文件归档
4. 补丁管理：用 `git format-patch` 或自定义脚本将二开修改与上游 diff 分离存储
```

### 2.2 优点

- ✅ **架构最简单** — 单一仓库、单一 `pip install`、单一 CI/CD 流水线，用户零配置
- ✅ **完全可控** — 可深度修改 `src/` 的任何行为，不受解耦约束
...
- ❌ **上游同步成本高** — 694 个被修改文件每次 merge 都可能冲突，需要人工逐一解决
...
| 首次冲突解决 | 处理 694 个被修改文件与上游最新版本的冲突 | 每个文件平均 5-50 行冲突解决代码，总计 ~5,000-15,000 行 | 最耗时步骤 |
| 每次后续同步 | 冲突解决取决于上游变更量，通常影响 10-50 个文件 | 每个周期 ~1,000-5,000 行 | 随上游活跃度变化 |
| 补丁文件维护 | 每次同步后更新补丁归档 | 每次 ~200-500 行 | 轻量 |

**关键结论**：初始迁移代码修改量约 **1,300 行**（配置+脚本+补丁管理），首次冲突解决需要处理 **~5,000-15,000 行**冲突区域。后续每次上游同步（按上游 1-2 月发布一次估算）需要处理 **~1,000-5,000 行**冲突。

### 2.5 适用场景

- 早期 MVP / 内部工具，快速上线第一优先级
- 团队 1-2 人，追求架构简单
- 上游变更不频繁，同步冲突可控时
...
**商业仓库不包含任何 `src/` 源码文件**，仅维护补丁文件（patch files）和二开模组代码。用户安装时，先通过 `pip install clawcodex` 或 `git clone upstream` 获取上游源码，然后应用补丁将二开修改叠加到上游源码之上。

工作流：
```
1. 商业仓库内容：clawcodex_ext/（补丁层） + extensions/（二开模组） + patches/（补丁文件）
2. 安装时：pip install clawcodex（上游源码）→ 拉取商业仓库 → 应用 patches/ → 安装 clawcodex_ext/ + extensions/
...
- ✅ **仓库精简** — 商业仓库仅含二开代码（~113,850 行大特性 + 基础设施），不含上游 179,062 行
- ✅ **知识产权隔离** — 商业代码与上游 MIT 代码物理分离，许可证边界清晰
- ✅ **上游同步零仓库冲突** — 上游更新不需要在商业仓库中解决 merge 冲突，只需更新补丁内容
- ✅ **三层解耦架构真正落地** — 现有架构设计（Layer 1 补丁层 + Layer 2 扩展层）正是为此策略服务
- ✅ **CI/CD 轻量** — 商业仓库 CI 只需跑补丁层和扩展层的测试

### 3.3 缺点

- ❌ **安装复杂度高** — 用户需要两步安装（上游 + 补丁 + 二开模组），需要安装脚本自动化
- ❌ **上游依赖脆弱性** — 商业产品运行时依赖上游仓库的可用性和版本兼容性
- ❌ **694 个被修改的 `src/` 文件需要补丁化** — 当前 18% 的上游文件被直接修改过，需要生成和维护对应补丁文件
- ❌ **补丁版本锁定** — 商业仓库必须锁定上游版本号，补丁只适配特定版本，版本升级时需要重新适配补丁
...
| 阶段 | 内容 | 代码修改量 | 备注 |
|------|------|-----------|------|
| 生成补丁文件 | 为 694 个被修改的 `src/` 文件生成补丁（`git format-patch`） | 自动生成，需人工复核 ~5,000-15,000 行补丁内容 | 一次性 |
...
**关键结论**：初始建立补丁系统的代码修改量约 **3,800-6,800 行**（脚本+回迁代码+补丁复核），补丁文件本身 **~5,000-15,000 行**（自动生成需人工复核）。后续每次上游版本升级需要 **~500-2,000 行**补丁更新。

### 3.5 适用场景

- 中型产品 / SaaS 服务，需要稳定的上游基础
- 团队 3-5 人，有精力维护上游兼容性
- 上游仍在活跃开发，需要定期同步
- 对知识产权隔离有严格要求（如合规审计）
...
## 4. 五模块脱离 clawcodex 可行性评估

### 4.1 核心问题

> 这五个模块是否可以脱离 clawcodex 应用到其他 agent 系统中（如 LangChain、AutoGen、CrewAI、OpenAI 等）？
...
class AgentRuntimeProtocol(Protocol):
    ...
    async def create_session(self, session_id: str) -> SessionHandle: ...
    async def restore_session(self, session_id: str) -> SessionHandle: ...
...
│                    五模块独立包（商业产品）                          │
│                                                                    │
│  clawcodex-orchestrator  clawcodex-sop  clawcodex-lkb             │
│  clawcodex-community-radar  clawcodex-visualizer                   │
│                                                                    │
│  依赖：clawcodex-core（纯抽象接口）+ 标准库                         │
│        ↑ 无任何 clawcodex 运行时依赖                               │
├──────────────────────────────────────────────────────────────────┤
│                    clawcodex-core（抽象接口层）                      │
│                                                                    │
│  AgentProtocol  ToolProtocol  EventProtocol  ProviderProtocol     │
│  PermissionProtocol  SkillProtocol  RecordingProtocol             │
│  DashboardProtocol  GitProtocol                                   │
│                                                                    │
│  纯 Protocol 定义，无任何实现代码，零依赖                           │
├──────────────────────────────────────────────────────────────────┤
│                    Agent Adapter 层                                │
│                                                                    │
│  clawcodex-adapter    langchain-adapter    autogen-adapter         │
│  crewai-adapter       openai-adapter       custom-adapter          │
│    (已有的)           (需新增)             (需新增)               │
│                                                                    │
│  实现：Protocol → 具体框架的调用                                    │
├──────────────────────────────────────────────────────────────────┤
│                    目标 Agent 系统                                 │
│                                                                    │
│  Claude Code    LangChain    AutoGen    CrewAI    OpenAI           │
│  (clawcodex)    (需适配)      (需适配)    (需适配)   (需适配)      │
└──────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────┐
│              抽象 CLI (clawcodex-cli)                              │
│                                                                    │
│  clawcodex orchestrator run --issue <id>                           │
│  clawcodex sop compile --workflow workflow.md                      │
│  clawcodex lkb decompose --task "..."                              │
│  clawcodex radar scan --sources github                             │
│  clawcodex viz dashboard --port 8080                               │
│                                                                    │
│  输入/输出格式标准化：JSON / NDJSON / YAML                          │
│  所有子命令通过统一入口调度，无需用户记住多个包名                     │
├──────────────────────────────────────────────────────────────────┤
│              抽象 MCP Server (clawcodex-mcp)                        │
│                                                                    │
│  tools:                                                             │
│    orchestrator_* / sop_* / lkb_* / radar_* / viz_*               │
│                                                                    │
│  通过 stdio 或 SSE 协议暴露给任何 MCP 客户端：                      │
│    Claude Desktop  /  VS Code  /  Cursor  /  其他 MCP host         │
│                                                                    │
│  特点：                                                             │
│    - 单进程启动，按需加载模块                                       │
│    - 支持热插拔：`--enable lkb,radar` 或 `--all`                    │
│    - 每个模块的 MCP 工具集可独立注册                                │
└──────────────────────────────────────────────────────────────────┘
```
...
| 维度 | 策略一：全部合入 + 定期同步 | 策略二：补丁安装（不上传源码） |
|------|:--------------------------:|:----------------------------:|
| **初始迁移代码修改量** | ~1,300 行（配置+脚本+补丁管理） | ~3,800-6,800 行（脚本+回迁+补丁复核） |
| **首次冲突解决** | ~5,000-15,000 行冲突区域 | 补丁文件 ~5,000-15,000 行（需人工复核） |
| **每次上游同步工作量** | ~1,000-5,000 行冲突解决 | ~500-2,000 行补丁更新 |
| **仓库体积** | 54 万行（含上游 33%） | 仅二开代码（~113,850 行 + 基础设施） |
| **知识产权隔离** | ❌ 边界模糊 | ✅ 物理隔离 |
| **安装复杂度** | 🟢 单仓库 `pip install` | 🟡 两步安装（上游 + 补丁 + 扩展） |
| **上游可用性依赖** | ✅ 不依赖上游仓库在线 | ❌ 依赖上游仓库可用性 |
| **调试难度** | 🟢 单一仓库，问题定位简单 | 🟡 需要区分上游/补丁/二开 |
...
同时验证两种策略的可行性：
1. 策略一：建立 `git remote add upstream` 同步机制，模拟一次 merge 冲突解决，评估实际冲突量
2. 策略二：对 1-2 个被修改的 `src/` 文件生成补丁，验证补丁安装流程的可行性

**产出**：两种策略的实际工作量基线，辅助决策选择。
...
| 选项 | 内容 | 代码修改量 |
|------|------|-----------|
| **走策略一** | squash 历史 → 建立同步机制 → 首次冲突解决 → 补丁管理脚本 | ~6,300-16,300 行 |
| **走策略二** | 生成补丁 → 编写安装脚本 → 回迁未补丁化修改 → 补丁适配脚本 | ~8,800-21,800 行 |

**选择建议**：
- 如果团队追求 **快速上线**（1-2 周出首个商业版本），且对知识产权隔离要求不高 → **策略一**
- 如果团队有 **合规审计需求**，需要清晰的知识产权边界，且能接受稍长的初始准备 → **策略二**
- 如果上游变更频繁（每月 1+ 次发布），策略二的长期维护成本更低 → **策略二**
...
| 活动 | 策略一 | 策略二 |
|------|--------|--------|
| 上游同步 | 每月 `git merge upstream/main` → 解决冲突 → 更新补丁归档 | 每季度检查上游版本 → 更新 patches/ 适配新 API |
| 同步工作量 | 每个版本 ~1,000-5,000 行冲突解决 | 每个版本 ~500-2,000 行补丁更新 |
| 测试验证 | 全量测试（含上游 3,884 个文件） | 仅测试二开代码（轻量） |
| 版本锁定 | 不锁定，随时可 merge | 锁定上游版本号，补丁只适配特定版本 |
...
注：`comm` = community_radar, `lkb` = logical_kanban, `viz` = visualizer, `orch` = orchestrator, `sop` = sop_converter
...
| 维度 | 策略一：全部合入 + 定期同步 | 策略二：补丁安装（不上传源码） |
|------|:--------------------------:|:----------------------------:|
| **初始迁移代码修改量** | ~1,300 行 | ~3,800-6,800 行 |
| **首次冲突/补丁工作量** | ~5,000-15,000 行冲突区域 | ~5,000-15,000 行补丁（需复核） |
| **每次上游同步工作量** | ~1,000-5,000 行冲突解决 | ~500-2,000 行补丁更新 |
| **仓库体积** | 54 万行（含上游 33%） | 仅二开代码 |
| **用户安装复杂度** | 🟢 1 个包 | 🟡 两步安装（上游+补丁+扩展） |
| **知识产权隔离** | 🔴 边界模糊 | 🟢 物理隔离 |
| **上游可用性依赖** | ✅ 不依赖上游在线 | ❌ 依赖上游仓库可用性 |
| **调试难度** | 🟢 单一仓库 | 🟡 需区分上游/补丁/二开 |
| **团队规模要求** | 1-2 人 | 3-5 人 |
| **适合的商业阶段** | 早期 MVP / 内部工具 | 中型产品 / SaaS 服务 |

---

> **文档维护**：本文档由 ClawCodex DevMind 团队维护，随架构演进持续更新。
> 最新版本请参见 `docs/COMMERCIALIZATION_PLAN.md`。