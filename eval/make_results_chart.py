...
def load(agent: str) -> dict:
    return json.loads(
        (SWEBENCH / f"{agent}-local.{RUN_ID}-{agent}.json").read_text(encoding="utf-8")
    )
...
ax1.bar(
    ...
    errors,
    w,
    bottom=[r + u for r, u in zip(resolved, unresolved)],
    label="Error",
    color=COL_ERR,
    edgecolor="white",
    linewidth=2,
...
delta_str = (
    f"clawcodex +{delta} ({100.0 * delta / total:+.1f} pp)"
    if delta > 0
    else f"clawcodex {delta} ({100.0 * delta / total:+.1f} pp)"
)
ax1.text(
    0.5,
    total + total * 0.01,
    ...
    va="bottom",
    fontsize=12,
    color=(COL_CC if delta > 0 else COL_OC),
    fontweight="bold",
