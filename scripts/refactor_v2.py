...
def main(ctx):
    ...
    refactored_path = Path(ctx['runDir']) / 'FEATURE_PLAN_REFACTORED_v2.md'
...
def merge_duplicates(features):
    ...
    for f_num, group in groups.items():
        ...
        # Keep the first as main, add others as sub-features
        main = group[0].copy()
        # For F-49, merge all three into one with sub-sections
        if f_num == 'F-49':
            ...
            # For other duplicates, keep first and extend content
            main['content'] = []
            for g in group:
                main['content'].extend(g['content'])
            # Re-extract status from merged content
...
def build_refactored_document(chapters, features):
    ...
    for ch in real_chapters:
        ...
        for feat in ch_feats:
            feat_anchor = f"f-{feat['f_num'].lower().replace('.', '-').replace('-', '')}"
...
def extract_first_paragraph(lines):
    ...
    for line in lines:
        text = line.strip()
        # Skip empty, markdown formatting, code, tables, etc.
        if not text:
            continue
