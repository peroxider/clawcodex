...
def main(ctx):
    ...
    for feat in features:
        out_lines.append(
            f"  {feat['f_num']}: {feat['title']} [{feat['status']['emoji']} {feat['status']['text']}] P={feat['priority']} len={feat['content_length']} ch={feat['chapter']}"
        )
...
def build_refactored_document(chapters, features):
    ...
    for ch in real_chapters:
        ch_anchor = (
            ch["title"].replace(" ", "-").replace("（", "").replace("）", "").replace("/", "")
        )
