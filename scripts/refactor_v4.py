...
def main():
    ...
    for i, line in enumerate(lines):
        ...
        if f_num and title and len(title) > 2:
            ...
            title = re.sub(r'\s*[（(](?:F-\d+)?\s*(?:✅|📋|🔄|🔭|⛔)?\s*[）)]\s*', '', title).strip()
            title = re.sub(r'\s*[（(][^）)]*(?:已完成|待开始|规划中|进行中|长期规划)[^）)]*[）)]\s*', '', title).strip()
    ...
    refactored_path = Path(r'C:\WorkSpace\clawcodex\docs\FEATURE_PLAN_REFACTORED_v4.md')
...
def extract_status(lines):
    status_map = {'✅': '已完成', '📋': '规划中', '🔄': '进行中', '🔭': '长期规划', '⛔': '已被取代'}
...
def rewrite_completed(feat):
    ...
    desc = extract_first_paragraph(feat['content'], feat['f_num'])
    if desc:
        out.append(desc)
    else:
...
def extract_first_paragraph(lines, f_num=None):
    ...
    for line in lines:
        ...
        if text.startswith('**') and text.endswith('**') and len(text) < 50:
            continue
        # Skip code patterns
        if text.startswith('class ') or text.startswith('def ') or text.startswith('import ') or text.startswith('from '):
            continue
        if text.startswith('return ') or text.startswith('self.') or text.startswith('raise ') or text.startswith('assert '):
            continue
        if text.startswith('pass') or text.startswith('break') or text.startswith('continue'):
            continue
        if text.startswith('{') or text.startswith('[') or text.startswith('}') or text.startswith(']'):
            continue
        if text.startswith('"') or text.startswith("'"):
            continue
        # Skip assignment lines (any line with = is likely code)
        if ' = ' in text:
            continue
        # Skip function/method calls (any function call pattern)
        if re.search(r'[a-zA-Z_]\w*(?:\.\w+)*\s*\(', text):
            continue
        # Skip list items
        ...
        if re.match(r'^\d+\.\s', text):
            continue
        # Skip lines with attribute access + brackets (array access)
        if re.search(r'\.[a-zA-Z_]\w*\[', text):
            continue
        # Skip lines with code keywords
        if re.search(r'\b(if|for|while|try|except|with|async|await|lambda)\b', text):
            continue
        # Skip lines that are just file paths or code-like references
        if re.match(r'^[a-zA-Z_][\w/]*\.[a-zA-Z]+\s*$', text):
            continue
        # Clean up
