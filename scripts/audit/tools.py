...
SNAPSHOT_PATH = (
    Path(__file__).resolve().parent.parent.parent / "src" / "reference_data" / "tools_snapshot.json"
)
...
def execute_tool(name: str, payload: str = "") -> ToolExecution:
    ...
    action = (
        f"Mirrored tool '{module.name}' from {module.source_hint} would handle payload {payload!r}."
    )
    return ToolExecution(
        name=module.name,
