...
def extract_goal(lines: list[str]) -> str:
    ...
    for line in lines:
        line = line.strip()
        if line.startswith('**目标**') or line.startswith('**目标**:') or line.startswith('目标：') or line.startswith('目标:'):
...
def rewrite_feature(feature: Feature) -> list[str]:
    ...
    if feature.status == '✅':
        ...
        for line in feature.content_lines:
            line = line.strip()
            if line and not line.startswith('#') and not line.startswith('>') and not line.startswith('|') and not line.startswith('```') and not is_archive_link(line):
                if len(line) > 20 and ('实现' in line or '完成' in line or '已' in line or '代码' in line or '行' in line):
                    impl_note = line
                    break
    ...
    for line in feature.content_lines:
        line = line.strip()
        if line and not line.startswith('#') and not line.startswith('>') and not line.startswith('|') and not line.startswith('```') and not line.startswith('`') and not line.startswith('**') and len(line) > 15:
            out.append(line)
            found_desc = True
            break
    if not found_desc:
    ...
    if feature.status == '🔄':
        ...
        for line in feature.content_lines:
            ...
            elif '已完成' in line and len(line) < 150 and not line.startswith('#') and not line.startswith('|'):
        ...
        for line in feature.content_lines:
            line = line.strip()
            if ('待' in line or '剩余' in line or 'TODO' in line or '待补' in line or '缺' in line) and len(line) < 150 and not line.startswith('#') and not line.startswith('|') and not line.startswith('```'):
    ...
    for line in feature.content_lines:
        line = line.strip()
        if line.startswith('|') and ('.py' in line or '.ts' in line or '.json' in line or '.yaml' in line or '.yml' in line):
...
def main():
    ...
    for f in features:
        out_lines.append(f"- [{f.f_number} {f.title}](#{f.f_number.lower().replace('.', '')}-{'-'.join(f.title.split()[:3])})")
    out_lines.append("")
    out_lines.append("---")
    out_lines.append("")
    ...
    out_lines.append("")
    out_lines.append("ClawCodex 是 Anthropic Claude Code 的 Python 移植版，同时扩展多 Provider 支持，目标成为功能完整的 AI Agent CLI 工具。")
    out_lines.append("")
    ...
    out_lines.append("```")
    out_lines.append("")
    out_lines.append("**核心约束**：所有 downstream/custom 开发默认进入 `clawcodex_ext/*`，`src/*` 仅接受 thin forwarding seams 和最小适配层。")
    out_lines.append("")
    out_lines.append("---")
    out_lines.append("")
    ...
    out_lines.append("|--------|------|:----:|:------:|")
    for f in features:
        out_lines.append(f"| {f.f_number} | {f.title} | {f.status} {f.status_text} | {f.priority} |")
    out_lines.append("")
