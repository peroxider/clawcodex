...
class StreamingToolExecutor:
    ...
    def _can_execute_tool(self, is_concurrency_safe: bool) -> bool:
        ...
        return len(executing) == 0 or (
            is_concurrency_safe and all(t.is_concurrency_safe for t in executing)
        )
    ...
    def _update_interruptible_state(self) -> None:
        ...
        if setter is None:
            return
        executing = [t for t in self._tools if t.status == "executing"]
        interruptible = len(executing) > 0 and all(
            self._get_tool_interrupt_behavior(t) == "cancel" for t in executing
        )
        try:
    ...
    async def get_remaining_results(self) -> AsyncGenerator[MessageUpdate, None]:
        ...
        while self._has_unfinished_tools():
            ...
            executing_promises = [
                t.promise for t in self._tools if t.status == "executing" and t.promise is not None
            ]
